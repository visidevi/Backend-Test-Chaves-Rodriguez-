from backend_test.settings import APP_TZ, NOTIFICATION_HOUR
...
def get_utc_for_specific_hour_in_timezone(tz: str = APP_TZ):
    santiago = get_datetime_for_custom_timezone(tz, hour=int(NOTIFICATION_HOUR), minute=0, second=0, microsecond=0)
    utc = santiago.astimezone(timezone("UTC"))
    return {
        "hour": utc.hour,
        "minute": utc.minute,
    }
...
class CelerySettings:
    ...
    CELERYBEAT_SCHEDULE = {
        "send_slack_reminder": {
            "task": "backend_test.tasks.send_slack_reminder",  # the same goes in the task name
            # timedelta(seconds=10) # crontab(hour=19, minute=26, day_of_week=1),
            "schedule": crontab(**get_utc_for_specific_hour_in_timezone())
        }
    }
...
app = Celery("backend_test", include=["backend_test.tasks"])
